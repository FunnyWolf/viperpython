...
class LLMModuleView(BaseView):
    def create(self, request, **kwargs):
        try:
            loadpath = request.data.get('loadpath')
            message = request.data.get('message')
            context = LLMModule.create(load_path=loadpath, message=message)
        except Exception as E:
            logger.exception(E)
            context = data_return(500, {}, CODE_MSG_ZH.get(500), CODE_MSG_EN.get(500))
        return Response(context)
    ...
    def destroy(self, request, pk=None, **kwargs):
        try:
            load_path = request.query_params.get('loadpath')
            context = LLMModule.destroy(load_path)
        except Exception as E:
            logger.exception(E)
            context = data_return(500, {}, CODE_MSG_ZH.get(500), CODE_MSG_EN.get(500))
        return Response(context)
