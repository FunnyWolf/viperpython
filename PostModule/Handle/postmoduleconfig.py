...
class PostModuleConfig(object):
    ...
    def get_one_module_config(modulename, module_files_dir="MODULES"):
        module_intent = PostModuleConfig.get_module_intent(modulename, module_files_dir)
    ...
    @staticmethod
    def get_llm_module_config(modulename, module_files_dir="MODULES_LLM"):
        module_intent = PostModuleConfig.get_module_intent(modulename, module_files_dir)
        if module_intent is None:
            return None
    ...
    @staticmethod
    def get_llm_module_object(modulename, module_files_dir="MODULES_LLM"):
        modulename = modulename.split(".")[0]
        if modulename == "__init__" or modulename == "__pycache__":  # __init__.py的特殊处理
            return None
        try:
            class_intent = importlib.import_module(f'{module_files_dir}.{modulename}')
            module_object = class_intent.PostModule()
    ...
    def load_all_modules_config():
        ...
        module_filenames = os.listdir(os.path.join(settings.BASE_DIR, 'MODULES'))
        for module_filename in module_filenames:
            module_name = module_filename.split(".")[0]
            one_module_config = PostModuleConfig.get_one_module_config(module_name, 'MODULES')
            if one_module_config is not None:
                all_modules_config.append(one_module_config)
                viper_module_count += 1
        logger.info(f"内置模块加载完成,加载{viper_module_count}个模块")
        ...
        # Web 模块
        web_module_count = 0
        module_filenames = os.listdir(os.path.join(settings.BASE_DIR, 'MODULES_WEB'))
        for module_filename in module_filenames:
            module_name = module_filename.split(".")[0]
            one_module_config = PostModuleConfig.get_one_module_config(module_name, 'MODULES_WEB')
            if one_module_config is not None:
                all_modules_config.append(one_module_config)
                web_module_count += 1
        logger.info(f"WEB模块加载完成,加载{web_module_count}个模块")
        ...
        # llm 模块
        llm_module_count = 0
        module_filenames = os.listdir(os.path.join(settings.BASE_DIR, 'MODULES_LLM'))
        for module_filename in module_filenames:
            module_name = module_filename.split(".")[0]
            one_module_config = PostModuleConfig.get_llm_module_config(module_name, 'MODULES_LLM')
            if one_module_config is not None:
                all_modules_config.append(one_module_config)
                llm_module_count += 1

            # one_module_object = PostModuleConfig.get_llm_module_object(module_filename, 'MODULES_LLM')
            # if one_module_object is not None:
            #     Xcache.update_module_object(one_module_object.loadpath, one_module_object)

        logger.info(f"AI模块加载完成,加载{llm_module_count}个模块")
        Notice.send_info(f"AI模块加载完成,加载{llm_module_count}个模块",
                         f"The AI built-in modules is loaded, {llm_module_count} modules has loaded")

        # 自定义模块
        # TODO 自定义模块当前只支持post类型
        diy_module_count = 0
        module_filenames = os.listdir(os.path.join(settings.BASE_DIR, 'Docker', "module"))
        for module_filename in module_filenames:
            module_name = module_filename.split(".")[0]
            one_module_config = PostModuleConfig.get_one_module_config(module_name, 'Docker.module')
            if one_module_config is not None:
                all_modules_config.append(one_module_config)
                diy_module_count += 1
        logger.info(f"自定义模块加载完成,加载{diy_module_count}个模块")
