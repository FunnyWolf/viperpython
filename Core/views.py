...
class NoticesView(BaseView):
    ...
    def create(self, request, pk=None, **kwargs):
        try:
            ...
            context = Notice.send_userinput(content=content, username=username)
...
class BaseAuthView(ModelViewSet, UpdateAPIView, DestroyAPIView):
    ...
    def create(self, request, pk=None, **kwargs):
        ...
        if Xcache.get_bruteforce_flag():
            context = data_return(303, null_response, BASEAUTH_MSG_ZH.get(303), BASEAUTH_MSG_EN.get(303))
            return Response(context)
...
class SettingView(BaseView):
    def list(self, request, **kwargs):
        ...
        context = Settings.list(kind=kind, tag=tag)
    ...
    def update(self, request, pk=None, **kwargs):
        ...
        tag = request.data.get('tag')
        setting = request.data.get('setting')
        context = Settings.update(kind=kind, tag=tag, setting=setting)
        return Response(context)
    ...
    def destroy(self, request, pk=None, **kwargs):
        ...
        context = Settings.destory(kind=kind, tag=tag, setting=setting)
        return Response(context)
