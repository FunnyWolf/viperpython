...
class APSWebModule(object):
    ...
    def putin_web_python_module_queue(self, web_module_intent=None):
        try:
            ...
            # 清空历史记录
            web_module_intent.clean_log()
            web_module_intent._module_uuid = module_uuid

            logger.warning(f"Web模块放入列表:{web_module_intent.NAME_ZH} uuid: {module_uuid}")
            self.ModuleJobsScheduler.add_job(func=web_module_intent._thread_run, max_instances=1, id=module_uuid)

            # 放入缓存队列,用于后续删除任务,存储结果等
            req = {
                'broker': web_module_intent.MODULE_BROKER,
                'uuid': module_uuid,
                'module': web_module_intent,
                'time': int(time.time()),
                'job_id': None,
            }
            Xcache.create_web_module_task(req)
    ...
    def store_executed_result(task_uuid=None):
        req = Xcache.get_web_module_task_by_uuid(task_uuid=task_uuid)
        if req is None:
        ...
        try:
            ...
            Notice.send_info(f"Web 模块: {module_common_instance.NAME_ZH} {module_common_instance.target_str} 执行完成",
                             f"Web Module: <{module_common_instance.NAME_EN}> {module_common_instance.target_str} start running")
            logger.warning(f"多模块实例执行完成:{module_common_instance.NAME_ZH}")
            Xcache.del_web_module_task_by_uuid(task_uuid=task_uuid)  # 清理缓存信息
            return True
        except Exception as E:
            Xcache.del_web_module_task_by_uuid(task_uuid=task_uuid)  # 清理缓存信息
    ...
    def delete_job_by_uuid(self, task_uuid=None):
        ...
        # 发送通知
        Notice.send_info(f"Web模块: {module_common_instance.NAME_ZH} {module_common_instance.target_str} 手动删除",
                         f"Web Module:<{module_common_instance.NAME_EN}> {module_common_instance.target_str} manually delete")
        logger.warning(f"多模块实例手动删除:{module_common_instance.NAME_ZH}")
        return True


aps_web_module = APSWebModule()
