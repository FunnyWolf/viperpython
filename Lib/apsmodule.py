...
class APSModule(object):
    ...
    def putin_post_python_module_queue(self, post_module_intent=None):
        try:
            ...
            logger.info(f"模块放入列表:{post_module_intent.NAME_ZH} uuid: {module_uuid}")
            self.ModuleJobsScheduler.add_job(func=post_module_intent.thread_run, max_instances=1, id=module_uuid)
    ...
    def store_executed_result(task_uuid=None):
        ...
        try:
            ...
            logger.info(f"多模块实例执行完成:{module_common_instance.NAME_ZH}")
    ...
    def delete_job_by_uuid(self, task_uuid=None):
        ...
        logger.info(f"多模块实例手动删除:{module_common_instance.NAME_ZH}")
        return True
