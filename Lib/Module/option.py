...
class _Option(object):
    def __init__(self, name,
                 ...
                 length=None, extra_data=None, options=None):
        ...
        self._options = options  # selectmulti类型的待选列表,如果type为selectmulti类型则此参数必须填写
    ...
    def to_dict(self):
        ...
        # 处理options参数的兼容性,请注意,此处无法处理handler和凭证等动态参数
        tmp_options_list = []
        for options in self._options:
            if isinstance(options, str) or isinstance(options, bytes):
                tmp_options_list.append({'name': options, 'value': options})
            else:
                if options.get('tag_zh') is not None and options.get('value') is not None:
                    tmp_options_list.append(options)
                else:
                    logger.warning(f"参数错误, name: {self._name} tag_zh:{self._tag_zh}")
        _dict['options'] = tmp_options_list
...
class OptionSelectMulti(_Option):
    """字符串类型参数"""
    ...
    def __init__(self, name,
                 ...
                 length=24, options=[]):
        super().__init__(type='select_multi', name=name, tag_zh=tag_zh, desc_zh=desc_zh, tag_en=tag_en, desc_en=desc_en,
                         required=required, default=default,
