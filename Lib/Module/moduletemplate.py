...
from Lib.configs import MSFLOOT, LLMMODULE_CHANNEL_GROUP, USER_INPUT
from Lib.customexception import CustomException
from Lib.file import File
from Lib.log import logger
from Lib.redisclient import RedisClient
from Lib.redissaver import RedisSaver
from Lib.webmoduletask import WebModuleTaskStatus
...
class _CommonModule(object):
    ...
    def generate_bypass_exe_file(self, template):
        ...
        filepath = FileMsf.write_loot_file(filename, bytedata)
        return filepath
...
class ProxyHttpScanModule(_CommonModule):
    ...
    def callback(self, request, response, data=None):
        """后台运行模块回调函数"""
        logger.info(self._custom_param)
        logger.info(request)
...
class BotMSFModule(_BotCommonModule):
    ...
    def callback(self, module_output):
        ...
        logger.info(self.mname)
        logger.info(self.opts)
        logger.info(module_output)
...
class WebPythonModule(_CommonModule):
    ...
    def __init__(self, project_id, input_list: list, custom_param):
        ...
        self.task_status = None  # 模块uuid

    def run(self):
        """后台运行模块回调函数"""
        logger.warning("you need to add run() function in you class")

    def _first_run(self):
        try:  # catch all exception from run()
            self.run()
            self.task_status = WebModuleTaskStatus.success
        except CustomException as E:
            self.task_status = WebModuleTaskStatus.error
            self.log_error(E.msg_zh, E.msg_en)
        except Exception as E:
            self.task_status = WebModuleTaskStatus.error
            self.log_except(E)
            logger.exception(E)
    ...
    def thread_run(self):
        t1 = ThreadWithExc(target=self._first_run)
        t1.start()
        while True:
            flag = Xcache.is_web_module_task_running(self._module_uuid)
            if not flag:  # 检查模块是否已经删除
                self.exit_flag = True
                time.sleep(1)
                t1.raise_exc(Exception)
                while t1.is_alive():
...
class LLMPythonModule(_CommonModule):
    MODULE_BROKER = BROKER.llm_python_module

    def __init__(self, custom_param):
        super().__init__(custom_param=custom_param)  # 父类无需入参
        self.llm = None
    ...
    def send_message(self, message: BaseMessage):
        ...
        async_to_sync(channel_layer.group_send)(
            f"{LLMMODULE_CHANNEL_GROUP}_{self.loadpath}",
            {
                'type': 'send_message',
    ...
    def list(self):
        config = {"configurable": {"thread_id": self.loadpath}}
        graph_state: StateSnapshot = self.graph.get_state(config)
    ...
    def delete(self):
        config = {"configurable": {"thread_id": self.loadpath}}
        delete_count = self.graph.checkpointer.delete(config)
        return True

    def run_graph(self):
        self.send_message(BaseMessage(type="status", content="running"))
        user_input = self.param(USER_INPUT)
        human_message = HumanMessage(content=user_input)
        config = {"configurable": {"thread_id": self.loadpath}}
        send_message_id_list = []
        for event in self.graph.stream({"messages": [human_message]}, config, stream_mode="values"):
            messages = event["messages"]
            last_send_message = messages[-1]
            if last_send_message.id not in send_message_id_list:
                self.send_message(last_send_message)
                send_message_id_list.append(last_send_message.id)
        self.send_message(BaseMessage(type="status", content="finished"))

    def run_graph_debug(self):
        self.delete()
        user_input = self.param(USER_INPUT)
        human_message = HumanMessage(content=user_input)
        config = {"configurable": {"thread_id": self.loadpath}}
        ...
        for event in self.graph.stream({"messages": [human_message]}, config, stream_mode="values"):
            ...
            if last_send_message.id not in send_message_id_list:
                send_message_id_list.append(last_send_message.id)
                send_message_list.append(last_send_message)
                print(send_message_list[-1])

    def export_graph_png(self):
        file_path = os.path.join('/root', 'output.png')

        with open(file_path, 'wb') as file:
            # 将图片数据写入文件
            file.write(self.graph.get_graph(xray=True).draw_mermaid_png())
