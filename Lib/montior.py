...
class MainMonitor(object):
    ...
    def start(self):
        ...
        # except socket.error:
        #     logger.error("MainMonitor 已经启动,请勿重复启动")
        #     return

        # 初始化配置
        try:
            logger.info("初始化ES配置")
            EsClient.add_default_connection()
            EsClient.init()
        except Exception as E:
            logger.exception(E)

        try:
            logger.info("初始化Project")
        ...
        # Xcache初始化部分
        logger.info("Xcache初始化")
        Xcache.init_xcache_on_start()

        # 加载模块配置信息
        logger.info("加载模块配置信息")
        ...
        # 定时清理日志
        self.MainScheduler.add_job(func=File.clean_logs, trigger='cron', day='1', hour='23', minute='59')
        self.MainScheduler.start()

        logger.info("初始化BotScheduler")
        ...
        # web module
        logger.info("初始化WebModuleScheduler")
        ...
        # 启动自动化定时任务
        logger.info("初始化postModuleSingletonScheduler")
