...
from Lib.configs import VIPER_SEND_SMS_CHANNEL, CN, EN, VIPER_WORK_CHANNEL
...
class Notice(object):
    ...
    @staticmethod
    def send_userinput(content, username="root"):
        """用户输入消息"""
        notice = {CN: content, EN: content, "level": 6, "time": int(time.time()), "username": username}
        Xcache.add_one_notice(notice)
        return notice
    ...
    @staticmethod
    def send_worker_task(task):
        rcon = RedisClient.get_result_connection()
        if rcon is None:
            return
        result = rcon.publish(VIPER_WORK_CHANNEL, json.dumps(task))
