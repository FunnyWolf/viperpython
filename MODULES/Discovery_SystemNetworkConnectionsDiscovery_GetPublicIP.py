...
class PostModule(PostMSFPowershellModule):
    NAME_ZH = "获取互联网出口IP"
    DESC_ZH = "模块通过访问https://ipwho.is/接口获取当前Session的互联网出口IP地址.\n" \
              "在使用云函数及CDN等技术隐藏C2的IP地址时,界面中显示Session的地理位置信息不准确,可以使用此模块获取准确信息."

    NAME_EN = "Obtain Internet outbound IP"
    DESC_EN = "Module request https://ipwho.is/ to obtain the Internet out IP address of the current session\n" \
    ...
    REFERENCES = ["https://attack.mitre.org/techniques/T1016/001/",
                  "https://ifconfig.me", "https://ipwho.is/"]
    AUTHOR = ["Viper"]
    ...
    def callback(self, status, message, data):
        if status:
            try:
                ...
                internet_ip = data.get("ip")

                ipaddress.IPv4Network(internet_ip)
            except Exception as E:
                self.log_error("获取信息错误", "Get output error")
                self.log_raw(data)
                return

            locate_zh = IPGeo.get_ip_geo_str(internet_ip, "zh-CN")
            locate_en = IPGeo.get_ip_geo_str(internet_ip, "en-US")

            self.log_good(f"出口IP: {internet_ip}", f"Outbound IP: {internet_ip}")
            self.log_good(f"地理位置: {locate_zh}", f"Location: {locate_en}")
            session = Session(self._sessionid)
            Notice.send_sms(
                f"SID: {session.sessionid}\nPlatform:{session.platform}\nInfo:{session.info}\n出口IP: {internet_ip}\n地理位置: {locate_zh}",
                f"SID: {session.sessionid}\nPlatform:{session.platform}\nInfo:{session.info}\nOutbound IP: {internet_ip}\nLocation: {locate_en}")
