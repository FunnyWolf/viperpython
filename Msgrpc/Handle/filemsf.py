...
class FileMsf(object):
    ...
    def list(filename=None, action=None):
        if filename is None:  # 列出所有文件
            result = FileMsf.list_loot_files()
            for one in result:
            ...
            binary_data = FileMsf.read_loot_file(filename)
    ...
    def create(file=None):
        result = FileMsf.write_file_to_loot(file)
        if result is True:
    ...
    def destory(filename=None):
        result = FileMsf.destory_loot_file(filename)
        if result is True:
    ...
    @staticmethod
    def write_file_to_loot(file=None):
        try:
            filename = file.name
    ...
    @staticmethod
    def write_loot_file(filename=None, data=None):
        filepath = File.safe_os_path_join(MSFLOOT, filename)
        with open(filepath, "wb+") as f:
            f.write(data)
    ...
    @staticmethod
    def read_loot_file(filename=None):
        filepath = File.safe_os_path_join(MSFLOOT, filename)
        if os.path.isfile(filepath):
    ...
    @staticmethod
    def destory_loot_file(filename=None):
        filepath = File.safe_os_path_join(MSFLOOT, filename)
        if os.path.isfile(filepath):
            os.remove(filepath)
            return True
