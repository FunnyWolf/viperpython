...
from Lib.api import data_return, is_ipaddress_port_in_use
...
class PortFwd(object):
    ...
    def list_portfwd():
        ...
        if result_list is None:
            ...
            default_lhost = Xcache.get_common_conf_by_key("lhost")
    ...
    def create(portfwdtype=None, lhost=None, lport=None, rhost=None, rport=None, sessionid=None):
        ...
        if flag is not True:
            return context
        lportint = int(lport)
        if is_ipaddress_port_in_use("0.0.0.0", lportint):
            # 端口已占用
            context = data_return(309, {}, PORTFWD_MSG_ZH.get(309), PORTFWD_MSG_EN.get(309))
            return context
